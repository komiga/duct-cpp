...
void validate(duct::Template const& tpl, Variable const& var, bool expected[4]) {
	...
	std::cout
		<<std::left
		<<"validate  : "<<std::setw(5)<<result[0]<<s_msg_expectation[expected[0]==result[0]]<<'\n'
		<<"  type    : "<<std::setw(5)<<result[1]<<s_msg_expectation[expected[1]==result[1]]<<'\n'
		<<"  identity: "<<std::setw(5)<<result[2]<<s_msg_expectation[expected[2]==result[2]]<<'\n'
		<<"  layout  : "<<std::setw(5)<<result[3]<<s_msg_expectation[expected[3]==result[3]]<<'\n'
	;
}

#define do_validation_single(tpl, var, v, vt, vi, vl) expected[0]=v; expected[1]=vt; expected[2]=vi; expected[3]=vl; validate(tpl, var, expected)

#define do_validation(series, vt_match, vi_match, vl_match, vt_not_match, vi_not_match, vl_not_match) { \
	std::cout<< "\n# " series ":\n"; \
...
int main(int, char*[]) {
	...
	// Layout - normal
	signature(tpl, duct::VARCLASS_COLLECTION, {}, {duct::VARCLASS_VALUE, duct::VARCLASS_VALUE, duct::VARCLASS_VALUE|duct::LAYOUT_FIELD_OPTIONAL});
