...
template<typename FP, std::size_t const s=sizeof(FP)> struct int_fp_type_impl;
template<typename FP> struct int_fp_type_impl<FP, 4> { typedef uint32_t type; };
template<typename FP> struct int_fp_type_impl<FP, 8> { typedef uint64_t type; };

template<typename FP> using int_fp_type=typename int_fp_type_impl<FP>::type;

auto constexpr ENSURE_SWAP=(duct::Endian::LITTLE==duct::Endian::SYSTEM) ? duct::Endian::BIG : duct::Endian::LITTLE;
...
void do_test_floating(T const value) {
	...
	auto yr=value; duct::byte_swap_ref_if(yr, ENSURE_SWAP);
	std::cout<<std::hex<<std::showbase
		<<"floating-point byte_swap   : "<<reinterpret_cast<cref_type>(value)<<" -> "<<reinterpret_cast<cref_type>(x)<<std::endl
		<<"floating-point byte_swap_if: "<<reinterpret_cast<cref_type>(value)<<" -> "<<reinterpret_cast<cref_type>(y)<<std::endl
		<<"floating-point byte_swap   : "<<reinterpret_cast<cref_type>(value)<<" -> "<<reinterpret_cast<cref_type>(xr)<<std::endl
		<<"floating-point byte_swap_if: "<<reinterpret_cast<cref_type>(value)<<" -> "<<reinterpret_cast<cref_type>(yr)<<std::endl
	;
}
